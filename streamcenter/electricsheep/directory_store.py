...
class DirStore:
    ...
    async def __aiter__(self):
        for file in self.root.iterdir():
            if '-' not in file.name:
                continue
            id, first, last = file.stem.split('-', 2)
            yield Sheep(id=int(id), first=int(first), last=int(last), path=file)

    async def add(self, sheep):
        """
        Adds the given sheep to the client
        """
        if list(self.root.glob(f"{sheep.id}-*")):
        ...
        async with aiohttp.ClientSession(raise_for_status=True) as session:
            async with session.get(sheep.url) as resp:
                ...
                filename = self.root / f"{sheep.id}-{sheep.first}-{sheep.last}{ext}"
